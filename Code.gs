const HEADERS = {
  ...
  MeritLogs: ['id', 'date', 'kind', 'action', 'repairAction', 'minutes', 'note', 'createdAt', 'updatedAt']
};
...
function ensureDefaultReflection_(ss) {
  ...
  if (existing.length) {
    ...
    const ownerRecord = current.id === 'default' || String(current.fullName || '').toLowerCase().indexOf('phạm nguyễn gia đạt') >= 0;
    if (ownerRecord && (birth !== '2003-05-12' || Number(current.lifePathNumber) !== 4)) {
      upsertRow_(sheet, Object.assign({}, current, { dateOfBirth: '2003-05-12', lifePathNumber: 4, updatedAt: new Date() }));
    }
    return;
  }
  upsertRow_(sheet, {
...
function installCalmCalendarReminders(events) {
  const calendar = CalendarApp.getDefaultCalendar();
  const rows = Array.isArray(events) ? events.slice(0, 45) : [];
  let created = 0;
  let skipped = 0;
  rows.forEach(function(row) {
    const start = new Date(row.start);
    if (isNaN(start.getTime())) return;
    const title = String(row.title || 'An Tâm · Nhắc thực hành').slice(0, 120);
    const sameDay = calendar.getEventsForDay(start);
    if (sameDay.some(function(event) { return event.getTitle() === title; })) {
      skipped++;
      return;
    }
    const end = new Date(start.getTime() + 15 * 60000);
    const event = calendar.createEvent(title, start, end, {
      description: 'Âm lịch ' + String(row.lunar || '') + '. Nhắc nhẹ từ My Assistant; bạn có thể sửa hoặc xoá trong Google Calendar.'
    });
