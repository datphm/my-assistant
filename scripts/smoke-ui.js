...
try {
  ...
  if (!getElement('assistantGateDisplay').innerHTML.includes('GATE A1 · 1/4') || getElement('assistantGateDisplay').innerHTML.includes('DISPLAY AT GATE ·') || !getElement('assistantGateDisplay').innerHTML.includes('VN999') || !getElement('assistantGateDisplay').innerHTML.includes('gate-status departed') || !getElement('assistantBrief').innerHTML.trim().startsWith('<section class="assistant-top-grid')) throw new Error('Active flight gate and sequence were not rendered correctly');
