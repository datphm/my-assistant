...
try {
  ...
  if (!getElement('flightJourneyStats').innerHTML.includes('FLIGHT LOG · AUTO TOTAL') || !getElement('flightJourneyStats').innerHTML.includes('miles') || !getElement('flightJourneyStats').innerHTML.includes('flightJourneyFact') || !getElement('flightJourneyFact').textContent.includes('around Earth')) throw new Error('Automatic flight lifetime totals did not render');
  if (!getElement('travelFootprint').innerHTML.includes('world-map-stage') || !getElement('travelFootprint').innerHTML.includes('vietnam-map-stage') || !getElement('travelFootprint').innerHTML.includes('🇨🇳') || !getElement('travelFootprint').innerHTML.includes('🇮🇩') || !getElement('travelFootprint').innerHTML.includes('🇰🇿') || !getElement('travelFootprint').innerHTML.includes('🇲🇾') || !getElement('travelFootprint').innerHTML.includes('🇹🇭') || !source.includes('footprintCountryAreas') || !source.includes('openFootprintPicker')) throw new Error('Stable real travel maps, flags, areas, or checkbox picker did not render');
  ...
  if (!indexSource.includes('id="flightJourneyStats"') || !styleSource.includes('Lightweight auto-updating flight lifetime totals') || !source.includes('flightJourneyFactTimer=setInterval')) throw new Error('Flight lifetime stats container, styling, or single timer missing');
  if (!styleSource.includes('Real, dependency-free travel maps') || !styleSource.includes('.footprint-picker-list') || !styleSource.includes('.world-map-stage')) throw new Error('Stable map and picker styles are missing');
